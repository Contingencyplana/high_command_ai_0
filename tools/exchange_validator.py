"""Exchange validator: verifies basic consistency of the shared exchange.

Checks:
- Orders in orders/pending must have ledger status "pending" and acks in acknowledgements/pending
- Orders in orders/dispatched must have ledger status "dispatched" and acks in acknowledgements/logged
- Ledger paths must exist on disk

Exit code is non‑zero if inconsistencies are found.
"""
