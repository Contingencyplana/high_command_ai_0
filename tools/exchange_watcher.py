...
CATEGORIES = {
    ...
    "reports_inbox": {
        ...
        "id_field": "report_id",
        "summary_field": "summary",
        "timestamp_field": "timestamp_submitted",
        "label": "Inbox reports",
    },
}
...
def load_snapshot(path: Path = STATE_PATH) -> Snapshot:
    ...
    try:
        ...
        raise ExchangeWatcherError(f"Corrupt watcher state at {path}: {exc}") from exc
...
def scan_category(category_name: str) -> Dict[str, Mapping[str, str]]:
    ...
    for candidate in sorted(root.glob("*.json")):
        try:
            ...
            # Skip malformed payloads but keep a hint in the snapshot so we do not lose track.
...
def process_once(quiet: bool = False) -> None:
    ...
    if emitted == 0 and not STATE_PATH.exists():
        # First run: provide a concise summary of what we can currently see.
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    if args.reset and STATE_PATH.exists():
        STATE_PATH.unlink()
        if not args.watch and not args.quiet:
            print("[exchange] Watcher state cleared.")
    ...
    try:
        if not args.watch:
            ...
            return 0
        if args.interval <= 0:
            raise ExchangeWatcherError("Polling interval must be positive when --watch is set")
        if not args.quiet:
            print(f"[exchange] Watching {EXCHANGE_ROOT} every {args.interval:.1f}s. Press Ctrl+C to stop.")
        while True:
            process_once(quiet=args.quiet)
            time.sleep(args.interval)
    except ExchangeWatcherError as exc:
        print(f"[exchange] error: {exc}", file=sys.stderr)
        return 1
    except KeyboardInterrupt:
        if not args.quiet:
            print("\n[exchange] Watcher stopped.")
        return 0
