...
| **GS** | Genesis Squad | Call-signed workspace cohorts (Alpha…Lambda) responsible for frontline execution. |
...
Operational clones that execute High Command doctrine on specific fronts. Each workspace carries a squad callsign (Alpha, Beta, Gamma, etc.) that matches frontline feedback terminology (`genesis-theta`, `genesis-zeta`). Callsigns travel with any embedded Alfas so reports can be traced back to their theatre.
...
Readiness posture where exchange syncs run in dual offline/online fashion to verify tooling before large pushes. All decision gates must report green in hybrid shadow mode before pivoting to mass deployment (see `logs/ops_readiness.json`).
...
Coordinated production wave (e.g., “Batch 1 (N=5)”) directed by Toyfoundry orders. Each batch freezes a baseline (`production/mass_alfa_batch1/baseline.md`), seeds Alfas, collects hello reports, and feeds a factory completion log back to High Command.
...
- **Exchange Sync Sweep (`python tools/exchange_all.py`)** — Runs the hybrid shadow sync (offline + online handoff) and writes results to `logs/exchange_all.json`.
- **Ops Readiness Sweep (`python -m tools.ops_readiness`)** — Evaluates readiness gates (civilian, ops, manufacturing) and records status to `logs/ops_readiness.json`.
...
- Note the Daily Doc Refresh slice or ledger entry that captured the update so hybrid shadow audits can trace when terminology entered circulation.
